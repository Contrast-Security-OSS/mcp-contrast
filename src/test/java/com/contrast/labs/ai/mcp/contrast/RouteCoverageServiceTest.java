...
/**
 * Comprehensive test suite for RouteCoverageService.
 * Tests the consolidated get_route_coverage method with different parameter combinations (AIML-224).
 *
 * The consolidated method replaces 6 previous methods:
 * - Unfiltered query: getRouteCoverage(appId, null, null, null)
 * - Session metadata filter: getRouteCoverage(appId, name, value, null)
 * - Latest session filter: getRouteCoverage(appId, null, null, true)
 */
...
class RouteCoverageServiceTest {
    ...
    void setUp() throws Exception {
        ...
        mockedSDKExtensionConstruction = mockConstruction(SDKExtension.class,
            (mock, context) -> {
                // Configure the mock to behave like mockSDKExtension
                when(mock.getRouteCoverage(anyString(), anyString(), any()))
                    .thenAnswer(invocation -> mockSDKExtension.getRouteCoverage(
                        invocation.getArgument(0),
                        invocation.getArgument(1),
                        invocation.getArgument(2)
                    ));
                when(mock.getRouteDetails(anyString(), anyString(), anyString()))
                    .thenAnswer(invocation -> mockSDKExtension.getRouteDetails(
                        invocation.getArgument(0),
                        invocation.getArgument(1),
                        invocation.getArgument(2)
                    ));
                when(mock.getLatestSessionMetadata(anyString(), anyString()))
                    .thenAnswer(invocation -> mockSDKExtension.getLatestSessionMetadata(
                        invocation.getArgument(0),
                        invocation.getArgument(1)
                    ));
            }
    ...
    private RouteCoverageResponse createMockRouteCoverageResponse(int routeCount) {
        ...
        for (int i = 0; i < routeCount; i++) {
            ...
            route.setExercised(i % 2 == 0 ? 1L : 0L); // Alternate between exercised (>0) and discovered (0)
            routes.add(route);
        }
    ...
    void testGetRouteCoverage_UnfilteredQuery_Success() throws Exception {
        ...
        // Act
        RouteCoverageResponse result = routeCoverageService.getRouteCoverage(TEST_APP_ID, null, null, null);
        ...
        verify(mockSDKExtension, times(3)).getRouteDetails(eq(TEST_ORG_ID), eq(TEST_APP_ID), anyString());
    }
    ...
    void testGetRouteCoverage_SessionMetadataFilter_Success() throws Exception {
        ...
        when(mockSDKExtension.getRouteCoverage(eq(TEST_ORG_ID), eq(TEST_APP_ID), any(RouteCoverageBySessionIDAndMetadataRequestExtended.class)))
            .thenReturn(mockResponse);
        ...
        // Act
        RouteCoverageResponse result = routeCoverageService.getRouteCoverage(
        ...
        verify(mockSDKExtension, times(4)).getRouteDetails(eq(TEST_ORG_ID), eq(TEST_APP_ID), anyString());
    }
    ...
    void testGetRouteCoverage_SessionMetadataFilter_MultipleRoutes() throws Exception {
        ...
        // Verify route details fetched for each route
        verify(mockSDKExtension, times(5)).getRouteDetails(eq(TEST_ORG_ID), eq(TEST_APP_ID), anyString());
    ...
    void testGetRouteCoverage_RouteDetailsFails() throws Exception {
        ...
        // Act & Assert
        IOException exception = assertThrows(IOException.class, () -> {
            routeCoverageService.getRouteCoverage(TEST_APP_ID, TEST_METADATA_NAME, TEST_METADATA_VALUE, null);
        });
    ...
    void testGetRouteCoverage_UseLatestSessionFalse() throws Exception {
        ...
        // Act
        RouteCoverageResponse result = routeCoverageService.getRouteCoverage(TEST_APP_ID, null, null, false);
