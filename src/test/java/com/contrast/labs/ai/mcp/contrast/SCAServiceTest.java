...
/**
 * Unit tests for SCAService (Software Composition Analysis).
 * Tests library retrieval and CVE vulnerability mapping.
 */
...
class SCAServiceTest {
    ...
    void setUp() {
        ...
        mockedSDKExtension = mockConstruction(SDKExtension.class,
            (mock, context) -> {
    ...
    void testGetApplicationLibrariesByID_Success() throws IOException {
        // Given
        List<LibraryExtended> mockLibraries = createMockLibraries(3);
        mockedSDKHelper.when(() -> SDKHelper.getLibsForID(
            eq(TEST_APP_ID), eq(TEST_ORG_ID), any(SDKExtension.class)
        )).thenReturn(mockLibraries);

        // When
        List<LibraryExtended> result = scaService.getApplicationLibrariesByID(TEST_APP_ID);
        ...
        mockedSDKHelper.verify(() -> SDKHelper.getSDK(
            eq(TEST_HOST), eq(TEST_API_KEY), eq(TEST_SERVICE_KEY),
            eq(TEST_USERNAME), eq(""), eq("")
        ));
        mockedSDKHelper.verify(() -> SDKHelper.getLibsForID(
            eq(TEST_APP_ID), eq(TEST_ORG_ID), any(SDKExtension.class)
        ));
    }
    ...
    void testGetApplicationLibrariesByID_EmptyList() throws IOException {
        ...
        List<LibraryExtended> emptyLibraries = new ArrayList<>();
        mockedSDKHelper.when(() -> SDKHelper.getLibsForID(
            eq(TEST_APP_ID), eq(TEST_ORG_ID), any(SDKExtension.class)
    ...
    void testGetApplicationLibrariesByID_SDKFailure() {
        ...
        mockedSDKHelper.when(() -> SDKHelper.getLibsForID(
            anyString(), anyString(), any(SDKExtension.class)
        ...
        assertTrue(exception.getMessage().contains("SDK connection failed"),
    ...
    void testGetApplicationLibrariesByID_VerifiesClassUsage() throws IOException {
        ...
        List<LibraryExtended> mockLibraries = createMockLibrariesWithClassUsage();
        mockedSDKHelper.when(() -> SDKHelper.getLibsForID(
            eq(TEST_APP_ID), eq(TEST_ORG_ID), any(SDKExtension.class)
        )).thenReturn(mockLibraries);
        ...
        assertTrue(result.get(0).getClassedUsed() > 0,
            "First library should have classes used");

        // Verify second library has class usage = 0 (likely unused)
        assertEquals(0, result.get(1).getClassedUsed(),
            "Second library should have zero classes used");
    }
    ...
    void testListCVESForApplication_Success() throws IOException {
        // Given
        CveData mockCveData = createMockCveDataWithApps();
        List<LibraryExtended> mockLibraries = createMockLibrariesWithClassUsage();

        // Mock SDKExtension.getAppsForCVE
        SDKExtension mockExtension = mock(SDKExtension.class);
        when(mockExtension.getAppsForCVE(eq(TEST_ORG_ID), eq(TEST_CVE_ID)))
            .thenReturn(mockCveData);
        ...
        mockedSDKExtension = mockConstruction(SDKExtension.class,
            (mock, context) -> {
                when(mock.getAppsForCVE(eq(TEST_ORG_ID), eq(TEST_CVE_ID)))
                    .thenReturn(mockCveData);
            });

        // Mock getLibsForID for class usage population
        mockedSDKHelper.when(() -> SDKHelper.getLibsForID(
            anyString(), eq(TEST_ORG_ID), any(SDKExtension.class)
        )).thenReturn(mockLibraries);

        // When
        CveData result = scaService.listCVESForApplication(TEST_CVE_ID);
    ...
    void testListCVESForApplication_NoCVEFound() throws IOException {
        ...
        CveData emptyCveData = new CveData();
        ...
        mockedSDKExtension = mockConstruction(SDKExtension.class,
            (mock, context) -> {
                when(mock.getAppsForCVE(eq(TEST_ORG_ID), anyString()))
                    .thenReturn(emptyCveData);
            });

        // When
        CveData result = scaService.listCVESForApplication("CVE-9999-NONEXISTENT");
    ...
    void testListCVESForApplication_ClassUsagePopulation() throws IOException {
        ...
        List<LibraryExtended> mockLibrariesForApp = createMockLibrariesWithMatchingHash();
    ...
    private List<LibraryExtended> createMockLibraries(int count) {
        List<LibraryExtended> libraries = new ArrayList<>();
        for (int i = 0; i < count; i++) {
            LibraryExtended lib = mock(LibraryExtended.class);
    ...
    private List<LibraryExtended> createMockLibrariesWithClassUsage() {
        ...
        LibraryExtended lib1 = mock(LibraryExtended.class);
        ...
        LibraryExtended lib2 = mock(LibraryExtended.class);
