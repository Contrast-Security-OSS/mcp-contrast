...
/**
 * Reusable test utilities for validating paginated responses.
 * Provides assertion methods to verify pagination behavior across all endpoints.
 */
public class PaginationTestHelper {
    ...
    public static <T> void assertNoValidationMessage(PaginatedResponse<T> response) {
        assertTrue(response.message() == null || response.message().isEmpty(),
    ...
    public static <T> void assertItemCount(PaginatedResponse<T> response, int expectedCount) {
        assertEquals(expectedCount, response.items().size(),
