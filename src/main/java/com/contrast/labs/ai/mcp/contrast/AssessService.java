...
import com.contrast.labs.ai.mcp.contrast.helpers.SessionFilteringHelper;
...
public class AssessService {
  ...
  /** Helper for session-based trace filtering. Initialized after @Value injection. */
  private SessionFilteringHelper sessionFilteringHelper;

  /** Initializes helper classes after Spring @Value injection is complete. */
  @PostConstruct
  void initHelpers() {
    this.sessionFilteringHelper = new SessionFilteringHelper(maxTracesForSessionFiltering);
  }
  ...
  public PaginatedResponse<VulnLight> searchAppVulnerabilities(
    ...
    try {
      ...
      if (needsInMemorySessionFiltering) {
        ...
        var filterPredicate =
            sessionFilteringHelper.buildSessionFilterPredicate(
                agentSessionId, sessionMetadataName, sessionMetadataValue);
        ...
        var fetchResult =
            sessionFilteringHelper.fetchTracesWithEarlyTermination(
