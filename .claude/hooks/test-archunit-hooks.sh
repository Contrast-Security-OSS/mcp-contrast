...
check "guard: Bash chained semicolon denied" archunit-store-guard.sh \
  "{\"tool_name\":\"Bash\",\"tool_input\":{\"command\":\"git status; echo pwned > $STORE\"}}" deny
check "guard: Bash chained && denied" archunit-store-guard.sh \
  "{\"tool_name\":\"Bash\",\"tool_input\":{\"command\":\"git status && echo pwned > $STORE\"}}" deny
check "guard: Bash chained || denied" archunit-store-guard.sh \
