...
Changed files face a second, stricter floor: any changed `src/main/java` file must reach 85% line coverage. CI enforces this on every pull request, and `make coverage-changed` runs the same check locally. `make install-hooks` installs an optional pre-push hook that runs it before you push. See `scripts/git-hooks/README.md` for detail.
