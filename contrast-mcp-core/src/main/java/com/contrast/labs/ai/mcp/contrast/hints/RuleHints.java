...
/**
 * Enum representing different rule types and their associated hints. These hints are used to
 * provide guidance to AI agents when generating responses.
 */
public enum RuleHints {
  ...
  GENERAL_GUIDANCE(
      Arrays.asList(
          "Where a vulnerable library exists in the stacktrace and it looks like that library may"
              + " be the cause of the underlying vulnerability, prioritise upgrading that library"
              + " but only fix the vulnerability explicitly asked by the developer",
          "Move the sanitisation/validation logic into a separate method to make it easier to add"
              + " as a security control if needed.",
          "If the fix does not break the data flow between user input and sink and doesnt use well"
              + " known libraries, you MUST give the user documentation on how to add a security"
              + " control. Do not forget to inform the developer of this as you fix the"
              + " vulnerability.",
          "If including regular expressions as a suggested fix, please advise that Contrast"
              + " requires configuration via Security Controls in order to recognize this fix."
              + " Documentation for Security Controls is available at"
              + " https://docs.contrastsecurity.com/en/security-controls.html",
          "Prefer to use industry best practices in remediating vulnerabilities from reputable"
              + " sources like OWASP.",
          "When using libraries for validation ensure they're well known and maintained e.g. like"
              + " dompurify, esapi, etc.")),

  HQL_INJECTION(
      "hql-injection",
      Arrays.asList(
          "If all permutations of an input variable are known, use a allow list / enum to break the"
              + " dataflow.",
          "If prepared statement already, it's likely that an injection exists in the string build"
              + " query prior to prepare statement.")),
  ...
  NOSQL_INJECTION(
      "nosql-injection",
      Arrays.asList(
          "If all permutations of an input variable are known, use an allow list / enum to break"
              + " the dataflow.")),

  NOSQL_INJECTION_DYNAMODB(
  ...
  CSRF(
      "csrf",
      Arrays.asList(
          "Requests that have header based authentication (jwt, API keys, etc) are not vulnerable"
              + " to CSRF. Java made improvements to agents to include this in analysis as of"
              + " 6.12.0.",
          "If possible enable CSRF protections that already exist in the application, e.g"
              + " Spring-Security if using Spring. If that is not possible consider CSRFGuard.")),

  PATH_TRAVERSAL(
      "path-traversal",
      Arrays.asList(
          "If possible do not put user input into the filename. Use a randomly generated string as"
              + " the file name.",
          "If user data is required in the file path, consider using the latest version of OWASP"
              + " ESAPI.")),
  ...
  CMD_INJECTION(
      ...
      Arrays.asList(
          "If all permutations of an input variable are known, use an allow list / enum to break"
              + " the dataflow.",
          "If possible use ProcessBuilder and pass each arguments in as individual elements in the"
              + " array e.g ProcessBuilder processBuilder = new ProcessBuilder(new"
              + " String[]{\"ls\",\"-l\",\"/tmp/\"}); Rather than concatenating arguments into a"
              + " single String.")),
  ...
  EXPRESSION_LANGUAGE_INJECTION(
      ...
      Arrays.asList(
          "If all permutations of an input variable are known, use an allow list / enum to break"
              + " the dataflow.",
          "escapeEcmaScript() in Apache string utils is not a valid sanitization for this"
              + " vulnerability.")),
  ...
  LDAP_INJECTION(
      ...
      Arrays.asList(
          "If all permutations of an input variable are known, use an allow list / enum to break"
              + " the dataflow.",
          "Consider using org.owasp.esapi.Encoder.encodeForLDAP(java.lang.String) for"
              + " sanitization.")),
  ...
  REFLECTED_XSS(
      ...
      Arrays.asList(
          "Consider using"
              + " org.owasp.validator.html.scan.AbstractAntiSamyScanner.scan(java.lang.String) for"
              + " sanitization.")),
  ...
  XPATH_INJECTION(
      "xpath-injection",
      Arrays.asList(
          "Consider using org.owasp.esapi.Encoder.encodeForXPath(java.lang.String) for"
              + " sanitization.")),

  OVERLY_PERMISSIVE_CROSS_DOMAIN_POLICY(
      "overly-permissive-cross-domain-policy", Collections.emptyList()),
  ...
  XXE(
      "xxe",
      Arrays.asList(
          "Where user input can enter into the XML being read ensure that External Entities are"
              + " disabled and ensure the document cannot supply its own doctype. e.g",
  ...
  UNTRUSTED_DESERIALIZATION(
      ...
      Arrays.asList(
          "For Java Object Deserialization use a java.io.ObjectInputFilter to create a whitelist of"
              + " known good classes that are expected.")),
  ...
  CRYPTO_BAD_MAC(
      "crypto-bad-mac",
      Arrays.asList(
          "If the crypto is not used for security sensitive functionality, this may not be an"
              + " issue.")),

  HARDCODED_KEY(
      "hardcoded-key",
      Arrays.asList(
          "Agents just look at the variable name to see if it seems like a \"key\" and if it's"
              + " statically defined, which can be wrong. With context, check if it makes sense.")),
  ...
  REQUEST_VALIDATION_CONTROL_DISABLED(
      "request-validation-control-disabled", Collections.emptyList()),
  ...
  HARDCODED_PASSWORD(
      "hardcoded-password",
      Arrays.asList(
          "Agents just look at the variable name to see if it seems like a \"password\" and if it's"
              + " statically defined, which can be wrong. With context, check if it makes sense.")),
  ...
  SSRF(
      "ssrf",
      Arrays.asList(
          "We should only report this if user input can control the protocol or host of a string"
  ...
  CRYPTO_BAD_CIPHERS(
      ...
      Arrays.asList(
          "If the crypto is not used for security sensitive functionality, this may not be an"
              + " issue.",
          "If the vulnerability exists in a 3rd party library, rather than accessible code,"
              + " consider upgrading that library.")),

  CRYPTO_WEAK_RANDOMNESS(
  ...
  CSP_HEADER_INSECURE(
      "csp-header-insecure",
      Arrays.asList(
          "We dislike how this rule works. IMO agents should just send up the CSP and it be"
              + " evaluated using https://csp-evaluator.withgoogle.com/, which is open source.")),

  CSP_HEADER_MISSING(
      "csp-header-missing",
      Arrays.asList(
          "Add guidance on what we look for in a secure CSP, so they don't add a CSP and then we"
              + " report the above rule.",
          "It's worth noting sometimes these headers could be set at the perimeter (app gateway,"
              + " load balancer, etc). It's probably not that common but could happen.")),
