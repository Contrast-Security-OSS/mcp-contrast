...
- **`protectPending` omitted from the contract**: TeamServer's `ServerJpaDao.findServersByFilters` projection selects `defend` into both the `defend` and `defendPending` slots (filed as TS-43252; previously reported in TS-20210 but never fixed), so the wire `defendPending` deterministically mirrors `defend` and carries no information. Rather than ship a field named `protectPending` that agents would misread as a real pending-restart flag, the contract omits it; the `@Tool` description states that Protect pending-restart state is not available from this endpoint. `assessPending` is projected correctly and remains in the contract. Re-adding `protectPending` is backward compatible once TS-43252 is fixed (or via a future `get_server`, whose single-entity path returns the true flag).
